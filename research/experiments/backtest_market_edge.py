"""Backtest market-edge betting strategies using calibrated model probabilities.

This script:
  - Loads the OOF evaluation predictions from the F_reg_2 model
...
Run:
  python3 features/backtest_market_edge.py
"""
...
def main():
    ...
    df = load_predictions_and_merge_odds(PRED_PATH, TRAIN_PATH)

    print("📊 Computing market-implied probabilities...")
    df["market_prob"] = compute_market_prob(df)
    df["edge"] = compute_edge(df["pred_win_prob_platt_oof"], df["market_prob"])

    print("📊 Summary statistics:")
    print(f"  Total horses: {len(df)}")
    print(f"  Total races: {df.groupby(['race_date', 'venue', 'race_no']).ngroups}")
    print(f"  Date range: {df['race_date'].min()} to {df['race_date'].max()}")
    print(f"  Model prob mean: {df['pred_win_prob_platt_oof'].mean():.4f}")
    print(f"  Market prob mean: {df['market_prob'].mean():.4f}")
    print(f"  Edge mean: {df['edge'].mean():.4f}")
    print(f"  Edge std: {df['edge'].std():.4f}")

    # Overall metrics per threshold
    summary_rows = []
    for threshold in THRESHOLDS:
        metrics = compute_betting_metrics(df, threshold)
    ...
    bucket_rows = []
    for threshold in THRESHOLDS:
        bucket_df = compute_odds_bucket_metrics(df, threshold)
        if not bucket_df.empty:
            bucket_rows.append(bucket_df)
    if bucket_rows:
        bucket_all = pd.concat(bucket_rows, ignore_index=True)
        bucket_all.to_csv(OUTPUT_DIR / "backtest_odds_buckets.csv", index=False)
        print("\n✅ Odds bucket metrics saved to: output/backtest/backtest_odds_buckets.csv")

    # Monthly ROI
    monthly_rows = []
    for threshold in THRESHOLDS:
        monthly_df = compute_monthly_roi(df, threshold)
        if not monthly_df.empty:
            monthly_rows.append(monthly_df)
    if monthly_rows:
        monthly_all = pd.concat(monthly_rows, ignore_index=True)
        monthly_all.to_csv(OUTPUT_DIR / "backtest_monthly_roi.csv", index=False)
        print("✅ Monthly ROI saved to: output/backtest/backtest_monthly_roi.csv")

    # Detailed bet-level results
    df.to_csv(OUTPUT_DIR / "backtest_bet_level_results.csv", index=False)
    print("✅ Bet-level results saved to: output/backtest/backtest_bet_level_results.csv")
