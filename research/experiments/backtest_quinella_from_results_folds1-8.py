"""Backtest Quinella betting using results table for actual finishing positions (Folds 1-8).

This script:
  - Loads race results from DuckDB (finish_pos)
  - Loads F_reg_2 win model predictions
  - Filters to CALIBRATION folds (1-8) instead of evaluation folds
  - Uses Platt calibrated probabilities if available, otherwise uses raw probabilities
...
Run:
  python3 features/backtest_quinella_from_results_folds1-8.py
"""
...
def load_win_predictions_with_draw(pred_path: Path, train_path: Path) -> pd.DataFrame:
    ...
    # Check which probability column to use
    if "pred_win_prob_platt_oof" in df.columns:
...
def backtest_quinella_race(
    ...
    # Compute model quinella probabilities (using specified probability column)
    ...
    # Return = sum of dividends for winning bets (dividends are already in $10 units)
...
def main():
    ...
    # Filter to CALIBRATION folds (1-8) - NOT evaluation folds
    print("📊 Filtering to CALIBRATION folds (1-8)...")
    win_df = win_df[(win_df["fold"] >= 1) & (win_df["fold"] <= 8)].copy()

    print(f"📊 Calibration folds (1-8): {len(win_df)} rows")

    # Determine which probability column to use
    prob_col = win_df["prob_col"].iloc[0] if "prob_col" in win_df.columns else "pred_win_prob_raw"
    ...
    # Backtest per race
    print("\n📊 Backtesting quinella betting (Folds 1-8)...")
    ...
    print("\n" + "=" * 80)
    print("🏁 QUINELLA BACKTEST SUMMARY — FOLDS 1-8 (CALIBRATION PERIOD)")
    print("=" * 80)
    print(
        summary[[
    ...
    # Save outputs
    bets_df.to_csv(OUTPUT_DIR / "quinella_bet_level_results_folds1-8.csv", index=False)
    summary.to_csv(OUTPUT_DIR / "quinella_backtest_summary_folds1-8.csv", index=False)

    print("\n✅ Outputs saved to:")
    print(" - output/quinella/quinella_bet_level_results_folds1-8.csv")
    print(" - output/quinella/quinella_backtest_summary_folds1-8.csv")

    print("\n" + "=" * 80)
    print("📊 COMPARISON: Folds 1-8 vs Folds 9-16")
    print("=" * 80)
    print("Folds 1-8 results are shown above.")
    print("Folds 9-16 results (corrected with $10 stake):")
    print("  Threshold 0.00: 339 bets, ROI +37.8%")
    print("  Threshold 0.02: 103 bets, ROI -22.4%")
    print("  Threshold 0.05:  47 bets, ROI +31.6%")
    print("  Threshold 0.08:  26 bets, ROI +50.0%")
