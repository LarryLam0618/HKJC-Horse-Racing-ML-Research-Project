...
"""Complete Quinella backtest with proper Platt calibrated probabilities.

This script:
  - Loads F_reg_2 predictions WITH Platt calibrated probabilities (folds 9-16 only)
  - Loads race results from DuckDB (finish_pos)
  - Loads dividends data
  - Computes model-implied quinella probabilities using Platt calibrated win probabilities
  - Computes market-implied quinella probabilities from dividends
  ...
  - Reports hit rate, ROI, profit/loss with $10 stake (HKJC minimum)

IMPORTANT: This script only works for folds 9-16 because only those folds have
           Platt calibrated probabilities (pred_win_prob_platt_oof).

Run:
  python3 features/backtest_quinella_complete.py
"""
...
# Use OOF eval file which has Platt calibrated probabilities
WIN_PRED_PATH = Path("output/regularization/F_reg_2_predictions_oof_eval.csv")
...
def load_win_predictions_with_draw(pred_path: Path, train_path: Path) -> pd.DataFrame:
    """Load win model predictions with Platt calibrated probabilities and merge with draw."""
    ...
    # Check which probability column is available
    if "pred_win_prob_platt_oof" in pred_df.columns:
        print("✅ Using Platt calibrated probabilities (pred_win_prob_platt_oof)")
        prob_col = "pred_win_prob_platt_oof"
    elif "pred_win_prob_raw" in pred_df.columns:
        print("⚠️ Using raw probabilities (pred_win_prob_raw) - no Platt calibration available")
        prob_col = "pred_win_prob_raw"
    else:
        raise ValueError("Prediction file has neither pred_win_prob_platt_oof nor pred_win_prob_raw")

    required_pred = ["race_date", "venue", "race_no", "horse_id", "win_label", prob_col, "fold"]
...
def backtest_quinella_race(
    ...
    # Compute model quinella probabilities (using Platt calibrated probabilities)
    win_probs = race_df.set_index("horse_id")[prob_col]
    ...
    # Each bet costs $10 (HKJC minimum stake)
    total_stake = float(bet_count) * 10.0

    # Return = sum of dividends for winning bets (dividends are in $10 units)
...
def main():
    ...
    print("📂 Loading win model predictions (with Platt calibration)...")
    win_df, prob_col = load_win_predictions_with_draw(WIN_PRED_PATH, TRAIN_PATH)
    ...
    print(f"📊 Evaluation folds (9-16): {len(win_df)} rows")
    ...
    # Backtest per race
    print("\n📊 Backtesting quinella betting (Folds 9-16, Platt calibrated)...")
    ...
    for (race_date, venue, race_no), results_race in results_by_race:
        ...
        for threshold in THRESHOLDS:
            metrics = backtest_quinella_race(
                race_df, results_race, dividends_race, draw_to_horse, threshold, prob_col
            )
    ...
    print("\n" + "=" * 80)
    print("🏁 QUINELLA BACKTEST SUMMARY — FOLDS 9-16 (PLATT CALIBRATED)")
    print("=" * 80)
    print(
        summary[[
    ...
    # Save outputs
    bets_df.to_csv(OUTPUT_DIR / "quinella_bet_level_results_complete.csv", index=False)
    summary.to_csv(OUTPUT_DIR / "quinella_backtest_summary_complete.csv", index=False)

    print("\n✅ Outputs saved to:")
    print(" - output/quinella/quinella_bet_level_results_complete.csv")
    print(" - output/quinella/quinella_backtest_summary_complete.csv")

    # Show best threshold
    if len(summary) > 0 and summary['bet_count'].sum() > 0:
        valid = summary[summary['bet_count'] >= 20]
        if len(valid) > 0:
            best = valid.loc[valid['roi'].idxmax()]
            print(f"\n🏆 Best threshold (min 20 bets): {best['threshold']:.2f}")
            print(f"   ROI: {best['roi']:.2%}")
            print(f"   Bets: {best['bet_count']}")
            print(f"   Hit rate: {best['hit_rate']:.2%}")
        else:
            print("\n⚠️  No threshold with >= 20 bets found")
